...
| 지오펜싱 푸시 알림 (3.3) | ✅ 실기기 확인 완료 — 권한 요청 UX 버그 수정 포함. 장기 배터리 소모 실측은 보류(사용자 판단) |
...
5. 전국 스팟 확장, UI 폴리싱 🟡 **진행 중** — 아래 5-A/5-B 완료, 확장·폴리싱은 계속
   ...
   - 5-B. data.go.kr 표준데이터셋 수집 배치 구축 → `pending`으로 적재 → 관리자 승인 → 앱 노출 (3.6) ✅ **완료** — 총 852건(1차 50건 + 추가 802건) 적재 완료, 관리자 승인 대기 중
...
**8번 실기기 검증에서 발견·해결한 버그:** "스팟 진입 알림" 토글이 위치·알림 권한을 이미 다 켰는데도 계속 "권한 필요"만 뜨고 켜지지 않는 문제 발견. 원인은 iOS가 위치 권한을 "앱 사용 중"과 "항상" 두 단계로 나누는데, 지오펜싱엔 "항상"이 필요하다는 걸 사용자가 알 수 없었던 것(에러 메시지가 뭉뚱그려져 있었음) — `requestGeofencePermissions()`가 foreground/background/notifications 항목별 결과를 반환하도록 바꾸고, 정확히 뭐가 빠졌는지("위치를 항상으로") 안내 + "설정으로 이동" 버튼(`Linking.openSettings()`)을 추가해 해결. 새 빌드로 실기기에서 정상 동작 확인됨.
...
- TourAPI(`TOUR_API_KEY`) — 신청 페이지(`data.go.kr/data/15101578/openapi.do`, 한국관광공사_국문 관광정보 서비스_GW) 활용신청 완료, 인증은 통과했으나 정확한 오퍼레이션 경로 확인 중. 신청 후 `TarRlteTarService1`(관광지별 연관 관광지 정보)만 우선 응답했는데, 이건 "관광지 간 연관성" 데이터라 우리가 원하는 설명·이미지 보완 목적과 안 맞아 **서비스 대상에서 제외**(2026-07-24 결정). 이미지·설명에 필요한 `KorService2` 계열은 아직 `Forbidden`(승인 반영 대기 중으로 추정) — 활성화 확인 후 재시도 필요
- ~~지오펜싱 권한 요청이 실기기에서 "권한 필요"만 반복 표시~~ → **해결**: "항상" 위치 권한이 필요하다는 걸 사용자가 알 수 없었던 UX 문제. 항목별 권한 상태 안내 + 설정 이동 버튼 추가로 실기기 확인 완료
- 지오펜싱 배터리 소모 이슈 — 권한 플로우는 실기기에서 정상 동작 확인. 장기간 사용 시 배터리 소모량 실측은 지금 당장 검증하기 어려워 **보류**(사용자 판단, 2026-07-24)
- ~~추천 화면에서 스팟 다수의 날씨를 병렬 조회하며 전체 목록 채워지기까지 오래 걸림~~ → **개선**: 반경/테마로 먼저 거른 스팟만 날씨를 조회하도록 순서 변경(`app/(tabs)/recommend.tsx`). 승인 스팟이 대폭 늘어나면(현재 852건이 승인 대기 중) 추가 최적화가 다시 필요할 수 있음
- **(신규)** EAS Build 애드혹 배포에 필요한 환경변수(Firebase·네이버지도·기상청·천문연구원·에어코리아 키 10개)는 `eas env:set`으로 `preview` 환경에 등록해뒀음 — 새 키가 추가되면 로컬 `.env`뿐 아니라 EAS 환경변수도 함께 갱신해야 클라우드 빌드에 반영됨
...
- ~~data.go.kr 나머지 약 800건을 더 수집할지 결정 필요~~ → **해결**: 전부 수집하기로 결정, 852건 적재 완료. 다음 병목은 관리자가 이 852건을 검토·승인하는 작업량 자체 — 대량 승인/반려를 더 편하게 하는 어드민 개선(예: 필터·검색)이 필요할 수 있음
- **(신규)** 실 서비스 전환 시점·범위 — 10장에 배포 대상만 정리해두고 착수는 보류 중. 법적 요건(위치기반서비스 이용약관, 개인정보처리방침), 앱스토어 계정, API 키 사용량 비용 등은 아직 검토 전

## 10. 실 서비스(프로덕션) 전환 계획

git(GitHub `teambase/photospot`)은 소스 보관용일 뿐, 실제로 서비스가 되려면 컴포넌트별로 별도 배포 대상이 필요하다. 10.1(실기기 테스트 배포)은 실제로 착수해 완료했고, 10.2(정식 서비스 전환)는 여전히 계획만 세워둔 상태다.

### 10.1 실기기 테스트 배포 (앱스토어 출시 전, 개인 테스트용) ✅ 완료 — 아이폰 설치 성공
...
- **권장**: 실기기에서 지속적으로 테스트하려면 Apple Developer Program 가입 + EAS Build 애드혹 배포가 맞다.

**실제 진행 기록 (2026-07-23):**
1. Apple Developer Program(Individual) 가입·승인 완료
2. `eas-cli` 설치, `eas login`, `eas init`으로 EAS 프로젝트 생성 — 단, `app.config.ts`가 동적 설정이라 `extra.eas.projectId`는 수동으로 넣어야 했음
3. `eas build:configure`로 `eas.json` 생성 (`preview` 프로필 = `distribution: internal`, 애드혹 배포용)
4. `eas device:create`로 아이폰 UDID 등록 (Apple ID 로그인 필요, 사용자가 직접 진행)
5. **막힘 1 — Bundle ID 충돌**: `com.photospot.app`이 이미 다른 Apple 계정에 등록되어 있어 사용 불가 → `com.teambase.photospot`으로 변경(`app.config.ts` ios/android 둘 다), 로컬 `ios/`도 `expo prebuild --clean`으로 재생성. **주의**: 네이버 지도 SDK client_id는 네이버 클라우드 콘솔에 등록된 Bundle ID와 일치해야 동작 — 콘솔에 새 Bundle ID를 추가 등록함
6. **막힘 2 — 클라우드 빌드는 로컬 `.env`를 못 읽음**: `.env`는 gitignore되어 EAS 클라우드 빌드 서버에 올라가지 않으므로, `app.config.ts`의 `process.env.X`가 전부 빈 값이 됨(네이버 지도 등 전부 미설정으로 뜸) → `eas env:set`으로 Firebase/네이버지도/기상청/천문연구원/에어코리아 키 10개를 EAS `preview` 환경변수로 별도 등록해 해결
7. `eas build --profile preview --platform ios` 실행 → 클라우드 빌드 성공 → 설치 링크로 **아이폰에 정상 설치 확인**
...
## 11. 향후 검토: 축제 데이터 연동 (설계 노트, 착수 전)

data.go.kr **전국문화축제표준데이터**(5장에 이름만 언급되어 있던 3개 후보 데이터셋 중 하나)를 스팟 소스로 추가하는 안. 사용자 검토(2026-07-24) 결과, 지금의 스팟 구조와 근본적으로 달라 별도 설계가 필요하다는 데 합의함 — **아직 착수하지 않음, 설계만 기록**.

**스팟과 다른 점 4가지:**
1. **수집 방식이 다름**: Open API가 아니라 **CSV 파일 다운로드** 방식 — 지금 `collector/`는 API 폴링 전용(`fetch` 기반)이라 CSV 파싱/임포트 경로를 별도로 만들어야 함
2. **데이터 성격이 다름**: 스팟은 승인되면 상시 노출이지만, 축제는 **기간(시작일~종료일) 동안만** 유효 — `Spot` 타입을 억지로 확장하기보다 **별도 컬렉션(`festivals`)으로 분리**하는 게 맞다고 판단
3. **어드민도 별도 관리 필요**: CSV 임포트 UI, 기간 표시·정렬 등 지금의 스팟 승인 큐(`admin/`)와는 다른 화면이 필요
4. **앱 UI도 별도 개발 필요**:
   - "축제"는 기존 6개 테마(일몰·일출/벚꽃/단풍/은하수·별/야경/안개)와 성격이 달라 테마 칩에 끼워넣기보다 **별도 아이콘/카테고리**로 다루는 게 자연스러움
   - 지도·추천 노출 로직에 **"오늘 날짜가 기간 안에 있는지" 필터**가 추가로 필요 — 지금 앱엔 이런 시간 기반 조건부 노출 개념 자체가 없음(승인 여부만 봄)

**착수 여부·시점 미정.** 실제로 진행하기로 하면 이 노트를 기준으로 8장 로드맵에 단계를 추가한다.
