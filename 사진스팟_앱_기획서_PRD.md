...
8. 실기기 지오펜싱 배터리·정확도 테스트 ⬜ 시뮬레이터는 백그라운드 위치 권한 플로우를 완전히 재현하기 어려워, 사용자가 아이폰에 직접 설치해 진행 예정
...
- 에어코리아(`AIRKOREA_API_KEY`), TourAPI(`TOUR_API_KEY`)도 미발급 — 대기질 판단, 표준데이터셋으로 못 채운 설명·이미지 보완이 이 키 발급에 걸려 있음
...
| 컴포넌트 | 배포 대상(안) | 비고 |
|---|---|---|
| 모바일 앱 (`app/`, RN+Expo) | EAS Build/Submit → App Store·Google Play | 개발자 계정 개설, 스토어 심사(백그라운드 위치 권한은 심사 까다로운 편) 필요. 정식 출시 전 TestFlight/내부 테스트 트랙으로 먼저 검증하는 것을 권장 |
